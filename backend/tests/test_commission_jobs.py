...
from hermes_finance import jobs
from hermes_finance import statements as st
...
def test_a_file_already_staged_is_not_staged_again(monkeypatch):
    ...
    from hermes_finance.statements import content_hash
...
def test_reconcile_links_before_it_rolls_up(monkeypatch):
    ...
    monkeypatch.setattr("hermes_finance.matching.relink_unmatched",
                        lambda *a, **k: (order.append("link"), Link())[1])
    monkeypatch.setattr("hermes_finance.reconcile.run_rollup",
...
def test_reconcile_reports_the_transitions_that_needed_no_new_money(monkeypatch):
    ...
    monkeypatch.setattr("hermes_finance.matching.relink_unmatched", lambda *a, **k: Link())
    monkeypatch.setattr("hermes_finance.reconcile.run_rollup", lambda *a, **k: Roll())
