...
- **Login behavior differs by build.** The public/dev build's `AuthGate` renders children with **no login screen** (it assumes an embedding portal owns auth). Without a session, Supabase RLS denies `commission_ledger` and the SPA shows `Load policies: permission denied for table commission_ledger`. For standalone local/cloud-agent work, set `VITE_AUTOLOGIN_EMAIL` / `VITE_AUTOLOGIN_PASSWORD` in `.env.local` to an `app_allowlist` user (same mechanism as the private tailnet build), then restart `npm run dev`. Restart after any `.env.local` change — Vite bakes these via `define`.
